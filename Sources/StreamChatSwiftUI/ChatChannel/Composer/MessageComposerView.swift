...
public struct MessageComposerView<Factory: ViewFactory>: View, KeyboardReadable {
  ...
  @EnvironmentObject var vm: ChatChannelViewModel
  
  var onMessageSent: () -> Void
  
  var canSendMessage: Bool {
    viewModel.channelController.channel?.canSendMessage != false
  }
    ...
    VStack(spacing: 0) {
      ...
      VStack(spacing: 0) {
        if !canSendMessage {
          ...
          .overlay(
            ZStack {
              if viewModel.dragStarted || viewModel.recordingState == .locked || viewModel.recordingState == .stopped {
